...
# FONT_PATH = os.path.normpath(os.path.join(os.path.dirname(__file__)))
...
class CustomTitleBar(QWidget):
    ...
    def mousePressEvent(self, event: QMouseEvent):
        if event.button() == Qt.MouseButton.LeftButton:
            self.drag_position = (
                event.globalPosition().toPoint() - self.parent.geometry().topLeft()
            )
            event.accept()
...
class PasswordBookGui(QMainWindow):
    ...
    def __init__(self, app: QApplication, logger):
        ...
        self.title_bar.setGeometry(
            0, 0, self.title_bar.size().width(), self.title_bar.size().height()
        )
