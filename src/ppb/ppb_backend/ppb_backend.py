...
class PasswordBookSystem:
    ...
    def load_encrypt(cls, file_path: str, encrypt_password: str):
        ...
        if type(file_data["encrypt_data"]) is str:
            data_bytes = decrypt_data(
                encrypted_data=base64.b64decode(file_data["encrypt_data"]),
                salt=base64.b64decode(file_data["encrypt_salt"]),  # pyright: ignore[reportArgumentType]
                nonce=base64.b64decode(file_data["encrypt_nonce"]),  # pyright: ignore[reportArgumentType]
            ...
            if type(data_bytes) is bytes:
                ...
                raise error_backend.BackendEncryptError("資料錯誤！")
        else:
            raise error_backend.BackendEncryptError("資料錯誤！")
    ...
    def save_encrypt(self, file_path: str, encrypt_password: str):
        if self._data is None:
            ...
            (data, salt, nonce) = self.encrypt_data(
            ...
            #     )
            # ).decode("utf-8")
            encrypt_data = base64.b64encode(data).decode("utf-8")
            encrypt_salt = base64.b64encode(salt).decode("utf-8")
            encrypt_nonce = base64.b64encode(nonce).decode("utf-8")
...
def decrypt_data(
    encrypted_data: bytes, salt, nonce, password: str
) -> Optional[bytes]:
    if enable_encrypt_feature is False:
        raise error_backend.BackendEncryptError("未安裝加密功能！")
    else:
